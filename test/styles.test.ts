...
describe('StyleTable: the number format a value brought with it', () => {
    function table(): StyleTable {
        return new StyleTable({ money: { numFmt: '#,##0.00' }, big: { size: 20 } });
    }

    it('leaves a value that brought no format to the style it was given', () => {
        const styles = table();
        assert.equal(styles.forValue(undefined, undefined), 0);
        assert.equal(styles.forValue(undefined, 'money'), styles.index('money'));
    });
    ...
    it('adds the format to a style that asked for nothing, so a date is not shown as a serial', () => {
        const styles = table();
        const dated = styles.forValue(DEFAULT_DATE_FORMAT, undefined);
    ...
    it('gives two formats two entries', () => {
        const styles = table();
        const dated = styles.forValue(DEFAULT_DATETIME_FORMAT, undefined);
        assert.notEqual(dated, styles.forValue(DEFAULT_DATE_FORMAT, undefined));
        assert.equal(numFmtIdOf(styles, dated), String(DEFAULT_DATETIME_FORMAT));
    });

    it('writes out a format code, which a built-in id has none of', () => {
        const styles = new StyleTable();
        styles.forValue('yyyy-mm-dd', undefined);
        styles.forValue('yyyy-mm-dd hh:mm:ss', undefined);
        const xml = styles.xml();
    ...
    it('keeps the rest of the style it was given, and only adds the format', () => {
        ...
        const dated = styles.forValue(DEFAULT_DATE_FORMAT, 'big');
    ...
    it('leaves a style that already says how to show its number alone', () => {
        ...
        const styles = table();
        assert.equal(styles.forValue(DEFAULT_DATE_FORMAT, 'money'), styles.index('money'));
    });

    it('asks for the style once, however many values bring the same format', () => {
        const styles = new StyleTable();
        for (let k = 0; k < 100; k++) styles.forValue(DEFAULT_DATE_FORMAT, undefined);
